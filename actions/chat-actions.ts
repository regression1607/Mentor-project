...
export async function getUserChats() {
  ...
  try {
    ...
    const query = currentUser.role === "mentor" ? { mentorId: currentUser._id } : { menteeId: currentUser._id }
    ...
    return chats.map((chat) => {
      ...
      const otherUser =
        currentUser.role === "mentor"
          ? { id: chat.menteeId._id.toString(), name: chat.menteeId.name, image: chat.menteeId.image }
          : { id: chat.mentorId._id.toString(), name: chat.mentorId.name, image: chat.mentorId.image }

      // Count unread messages
      const unreadCount = chat.messages.filter((msg) => !msg.read && !msg.sender.equals(currentUser._id)).length
...
export async function getChatById(chatId: string) {
  ...
  try {
    ...
    // Find the chat
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const chat = await Chat.findById(chatId).populate("mentorId", "name image").populate("menteeId", "name image") as any
    ...
    // Check if user is part of this chat
    if (!chat.mentorId._id.equals(currentUser._id) && !chat.menteeId._id.equals(currentUser._id)) {
      return null
    }
